...
Car::Car()
    : reg_number("1111 AA-1"), year_of_release(1900), body_color("black") {
}
Car::Car(std::string reg_number, int release_year, std::string body_color)
    : reg_number(reg_number), year_of_release(release_year),
    body_color(body_color) {
}
Car::Car(const Car& car) {
    this->reg_number = car.reg_number;
    this->year_of_release = car.year_of_release;
    this->body_color = car.body_color;
}
Car::~Car() { year_of_release = 0; }

std::string Car::get_reg_number() { return reg_number; }
...
int Car::get_year_of_release() { return year_of_release; }
void Car::set_year_of_release(int year) { year_of_release = year; }
std::string Car::get_body_color() { return body_color; }
...
std::istream& operator>>(std::istream& in, Car& car) {
    ...
    std::cout << "Enter the car's year of "
        "manufacture(MIN_YEAR_RELEASE-MAX_YEAR_RELEASE)\n";
...
std::ostream& operator<<(std::ostream& out, const Car& car) {
    out << "| Registration number  " << car.reg_number << "| Year of release  "
        << car.year_of_release << "| Body color ";
...
std::fstream& operator>>(std::fstream& in, Car& car) {
    ...
    in.read(reinterpret_cast<char*>(&car.year_of_release),
        sizeof(car.year_of_release));
...
std::fstream& operator<<(std::fstream& out, Car& car) {
    int reg_number_size = car.reg_number.size();
    out.write(reinterpret_cast<const char*>(&reg_number_size),
        sizeof(reg_number_size));
    out.write(car.reg_number.c_str(), reg_number_size);

    out.write(reinterpret_cast<const char*>(&car.year_of_release), sizeof(int));

    int color_size = car.body_color.size();
